...
df_weekly = df.groupby("day_of_week", as_index=False).agg({"number_of_persons_injured": "sum"})

# Apply Sunday fix: Divide Sunday's injuries by 2
df_weekly.loc[df_weekly["day_of_week"] == "Sunday", "number_of_persons_injured"] /= 2
...
Here we demonstrate a two factor facet, splitting cases
occuring in the daytime, and at night.
```{python}
#| output-location: slide
#| echo: true
...
(ggplot(df, aes(x="day_of_week", y="number_of_persons_injured", fill="day_of_week")) + 
 geom_bar(stat="identity") +
 facet_grid("borough ~ time_of_day") +  # Rows = borough, Columns = time of day
 scale_fill_brewer(type="qual", palette="Set1") +
